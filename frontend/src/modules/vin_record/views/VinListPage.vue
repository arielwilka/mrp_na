  ...
  <div class="list-page">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-else v-for="item in records" :key="item.id" :class="{ selected: selectedIds.includes(item.id) }">
              ...
              <td data-label="Type">{{ item.vehicle_type_name || '-' }}</td>
    ...
    <div v-if="isEditModalOpen" class="modal-backdrop" @click.self="closeEditModal">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-row">
                <div class="form-group"><label>Full VIN</label><input v-model="editForm.full_vin" disabled class="input-disabled" /></div>
            </div>
            <div class="form-group">
                <label>Tahun</label>
                <select v-model="editForm.production_year"><option v-for="y in masterData.years" :key="y.id" :value="y.id">{{ y.year }}</option></select>
            </div>
            <div class="form-group">
                <label>Tipe</label>
                <select v-model="editForm.vehicle_type"><option v-for="t in masterData.types" :key="t.id" :value="t.id">{{ t.name }}</option></select>
            </div>
            <div class="form-row">
                <div class="form-group"><label>Varian</label><select v-model="editForm.variant"><option :value="null">-</option><option v-for="v in editTypeOptions.variants" :key="v.id" :value="v.id">{{ v.name }}</option></select></div>
                <div class="form-group"><label>Warna</label><select v-model="editForm.color"><option :value="null">-</option><option v-for="c in editTypeOptions.colors" :key="c.id" :value="c.id">{{ c.name }}</option></select></div>
            </div>
            <div class="warning-box">⚠️ Hanya update data administrasi.</div>
        </div>
        <div class="modal-footer">
...
const printTargetItem = ref<any>(null); // Jika null, berarti Bulk Print
...
// Filters & Pagination
const filters = reactive({ search: '', type: '', year: '' });
...
const editTypeOptions = computed(() => {
  const typeObj = masterData.types.find((t: any) => t.id === editForm.vehicle_type);
...
onMounted(async () => {
  try {
    const [t, y, tmpl] = await Promise.all([ 
        axios.get(`${API_URL}/types/`), axios.get(`${API_URL}/years/`), axios.get(`${API_URL}/templates/`)
    ]);
...
const fetchData = async () => {
  isLoading.value = true; selectedIds.value = []; 
  try {
    const actualSize = pagination.isCustom ? customSizeInput.value : pagination.pageSize;
    const params = { page: pagination.current, page_size: actualSize, search: filters.search, vehicle_type: filters.type, production_year: filters.year };
    const res = await axios.get(`${API_URL}/records/`, { params });
    
    records.value = res.data.results.map((r: any) => ({
      ...r,
      vehicle_type_name: r.vehicle_type_name || '-', variant_name: r.variant_name || '-', color_name: r.color_name || '-', created_by_name: r.created_by_name || 'System'
    }));
    pagination.total = res.data.count; pagination.next = res.data.next; pagination.prev = res.data.previous;
    pagination.from = (pagination.current - 1) * actualSize + 1; pagination.to = Math.min(pagination.current * actualSize, pagination.total);
...
const handleDelete = async (id: number) => { if (confirm('Hapus?')) { await axios.delete(`${API_URL}/records/${id}/`); fetchData(); } };
const handleBulkDelete = async () => { if (confirm(`Hapus ${selectedIds.value.length} item?`)) { for (const id of selectedIds.value) await axios.delete(`${API_URL}/records/${id}/`); fetchData(); } };

// --- LOGIC PRINTING (MODAL BASED) ---
const openPrintModal = (item: any = null) => {
    printTargetItem.value = item; // Jika null = Bulk Mode
...
const confirmPrint = async () => {
    ...
    try {
        ...
        for (const item of items) {
            // UBAH DISINI: Kirim 'item' (object), bukan 'item.full_vin' (string)
            await renderAndSend(item); 
            
            await new Promise(r => setTimeout(r, 300));
        }
        isPrintModalOpen.value = false;
        alert("Perintah cetak terkirim!");
    } catch(e) {
...
const renderAndSend = async (item: any) => {
     // Setup data ke state reactive
     printData.full_vin = item.full_vin;
     printData.type_name = item.vehicle_type_name || '-';
...
// --- EDIT ---
const handleEdit = (item: any) => { editForm.id = item.id; editForm.full_vin = item.full_vin; editForm.serial_number = item.serial_number; editForm.production_year = item.production_year; editForm.vehicle_type = item.vehicle_type; editForm.variant = item.variant; editForm.color = item.color; isEditModalOpen.value = true; };
const closeEditModal = () => isEditModalOpen.value = false;
const saveEdit = async () => { if (!editForm.id) return; isSavingEdit.value = true; try { await axios.patch(`${API_URL}/records/${editForm.id}/`, { vehicle_type: editForm.vehicle_type, variant: editForm.variant, color: editForm.color, production_year: editForm.production_year }); alert("Updated!"); closeEditModal(); fetchData(); } catch (e) { alert("Error update"); } finally { isSavingEdit.value = false; } };
...
.title { font-size: 1.5rem; margin: 0; color: var(--text-primary); }
.subtitle { color: var(--text-secondary); margin: 5px 0 0; font-size: 0.9rem; }
