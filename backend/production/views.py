...
class ShopFloorViewSet(viewsets.ViewSet):
    ...
    def scan_unit(self, request):
        ...
        # Cari Unit: Prioritas Full VIN, fallback Internal ID
        ...
        if not unit: return Response({"detail": "Unit tidak ditemukan."}, status=404)

        # Cek Requirements Traceability
        requirements = []
        if station_id:
            ...
            if active_route:
                ...
                if step_config:
                    for part in step_config.traceability_triggers.all():
                        is_installed = PartInstallationLog.objects.filter(unit=unit, part_master=part).exists()
                        requirements.append({
                            "part_name": part.part_name, "part_number": part.part_number,
                            "is_scanned": is_installed, "scanned_value": "INSTALLED" if is_installed else "-"
                        })

        serializer = ProductionUnitSerializer(unit)
        return Response({"unit": serializer.data, "requirements": requirements})

    # --- B. SCAN PART (POLISI INPUT) ---
    @action(detail=False, methods=['post'])
    def scan_part(self, request):
        ...
        if not all([unit_id, station_id, scanned_barcode]): return Response({"detail": "Data tidak lengkap."}, status=400)
        ...
        if not step_config: return Response({"detail": "Station ini tidak terdaftar dalam Route."}, status=400)

        required_parts = step_config.traceability_triggers.all()
        if not required_parts.exists(): return Response({"detail": "Station ini tidak membutuhkan scan part."}, status=400)
        ...
        for part in required_parts:
            ...
            if part.is_qc_required:
                ...
                if item:
                    matched_part = part; inventory_item = item; break 
            # JALUR NON-QC
            else:
        ...
        if not matched_part: return Response({"detail": f"Barcode '{scanned_barcode}' tidak valid atau belum lolos QC."}, status=400)

        # 3. Validasi Inventory Gatekeeper
        if matched_part.is_qc_required:
            if not inventory_item: return Response({"detail": "Part QC tidak ditemukan."}, status=400)
            if inventory_item.current_status != 'OK': return Response({"detail": f"GAGAL: Status part '{inventory_item.current_status}'."}, status=400)

        # 4. Validasi Unique Serial
        ...
        try:
            with transaction.atomic():
                if inventory_item:
                    ...
                    item_locked.current_status = 'USED'; item_locked.save()
    ...
    def process_station(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                # REFRESH ORDER DATA AGAR WIP COUNTER AKURAT
                ...
                if action_type == 'PASS':
                    if station.is_finish_point:
                        ...
                        # SAFETY CHECK: HANYA KURANGI JIKA SALDO > 0
                        # Mencegah error "CHECK constraint failed" (Negative Value)
                ...
                # Batch Record
                batch_rec, _ = StationBatchRecord.objects.get_or_create(production_order=unit.origin_order, station=station)
